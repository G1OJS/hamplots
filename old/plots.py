    ...
    for RxTx in ["Rx","Tx"]:
        ...
        for band in myBands:
            for mode in myModes:
                ...
                print(f"{len(remote_calls) if (remote_calls) else 0} remote calls")
                remote_calls = cover_home_calls(remote_calls, homecall_reports)
                print(f"{len(remote_calls) if (remote_calls) else 0} remote calls needed for at least one spot per home call")
